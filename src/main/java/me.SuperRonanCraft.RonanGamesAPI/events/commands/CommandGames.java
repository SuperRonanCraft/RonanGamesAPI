...
public class CommandGames implements CommandExecutor, TabCompleter {
    ...
    private final List<CommandGamesExpansion> expCmd = new ArrayList<>();
    ...
    void addExpansionCmd(CommandGamesExpansion cmd) {
        expCmd.add(cmd);
        RonanGamesCorePlugin.getInstance().getLogger().info("Hooked " + cmd.getExpansion().getNameCustom() + " for command listener!");
    }
